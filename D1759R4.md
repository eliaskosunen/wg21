...
_Note to editor:_ Replace the ? with the appropriate section number. As of [@N4849], that would be 29.9.2.
...
> A *native handle* can be obtained with the member function `native_handle()`, which is invalidated by a 
...
> \pnum{6} 
> 
> \pnum{7} [ *Note ?:* This differs from the native handles of `thread`, `mutex` and `condition_variable` [thread.req.native],
